...
export function getRetryOptions(retries, exemptStatusCodes, defaultOptions) {
  if (retries <= 0) {
  ...
  const requestOptions = { ...defaultOptions.request, retries }

  core.debug(
    `GitHub client configured with: (retries: ${
      requestOptions.retries
    }, retry-exempt-status-code: ${
      retryOptions.doNotRetry
        ? retryOptions.doNotRetry
...
export function parseNumberArray(listString) {
  if (!listString) {
    return []
  }
